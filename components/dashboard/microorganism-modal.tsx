...
import MicroorganismForm from './microorganism-form'
import { Microorganism } from '@/types'

interface MicroorganismModalProps {
  mode: 'add' | 'edit'
  microorganism?: Microorganism
  trigger?: React.ReactNode
}

export function MicroorganismModal({ mode, microorganism, trigger }: MicroorganismModalProps) {
      ...
      <DialogTrigger asChild>
        {trigger || (
          <Button className="bg-jkuat-green hover:bg-jkuat-green-dark text-white font-semibold rounded-xl shadow-lg px-8">
            {mode === 'add' ? 'Add Microorganism' : 'Edit Microorganism'}
          </Button>
        )}
      </DialogTrigger>
      <DialogContent className="sm:max-w-[95vw] md:max-w-2xl lg:max-w-4xl max-h-[90vh] overflow-y-auto rounded-xl border-jkuat-gray-100 shadow-2xl custom-scrollbar">
        <DialogHeader className="border-b border-jkuat-gray-100 pb-4 mb-4">
          <DialogTitle className="text-2xl font-extrabold text-jkuat-gray-900 tracking-tight">
             {mode === 'add' ? 'New microbial strain' : 'Edit strain record'}
          </DialogTitle>
        </DialogHeader>

        <MicroorganismForm
          initialValues={microorganism}
          submitUrl={mode === 'add' ? '/api/microorganisms' : `/api/microorganisms/${microorganism?.id}`}
